...
local function RegisterStashTarget()
    ...
    exports['qb-target']:AddBoxZone(stashTargetBoxID, vector3(stashLocation.x, stashLocation.y, stashLocation.z), 100, 100, {
        name = boxName,
        heading = 0.0,
        debug = true,
        distance = 1000,
...
local function RegisterOutfitsTarget()
    ...
    exports['qb-target']:AddBoxZone(outfitsTargetBoxID, vector3(outfitLocation.x, outfitLocation.y, outfitLocation.z), 100, 100, {
    ...
    }, {
        {
            ...
            type = 'server',
            event = 'qb-houses:server:openOutfits',
        },
    })
end
...
local function RegisterCharactersTarget()
    ...
    exports['qb-target']:AddBoxZone(charactersTargetBoxID, vector3(logoutLocation.x, logoutLocation.y, logoutLocation.z), 100, 100, {
...
local function RegisterHouseEntranceZone(id, house)
    ...
    if exports['qb-core']:GetPlayerData().job and exports['qb-core']:GetPlayerData().job.name == 'realestate' then
        isOwned = true
    end
    ...
    if not isOwned then
        ...
        elseif isOwned and not HasHouseKey then
            ...
            if exports['qb-core']:GetPlayerData().job and exports['qb-core']:GetPlayerData().job.type == 'leo' then
                options[#options + 1] = {
                    icon = 'fas fa-lock',
...
local function DeleteBoxTarget(boxId)
    if not boxId then return end
    exports['qb-target']:RemoveZone(boxId)
end
...
local function SetClosestHouse()
    local pos = GetEntityCoords(GetPlayerPawn())
    local current = nil
    local dist = nil
    if not IsInside then
        ...
        if ClosestHouse ~= nil and tonumber(dist) < 30 then
            TriggerCallback('ProximityKO', function(key, owned)
                HasHouseKey = key
                isOwned = owned
            end, ClosestHouse)
        end
    ...
    TriggerLocalClientEvent('qb-garages:client:setHouseGarage', ClosestHouse, HasHouseKey)
end

local function setHouseLocations()
    if ClosestHouse ~= nil then
        TriggerCallback('getHouseLocations', function(result)
            if result ~= nil then
...
--         TriggerCallback('getHouseDecorations', function(result)
...
local function CheckDistance(target, distance)
    ...
    return GetDistanceBetweenCoords(pos, target) <= distance
end
...
local function getKeyHolders()
    ...
    local p = promise.new()
    TriggerCallback('getHouseKeyHolders', function(holders)
        p:resolve(holders)
    end, ClosestHouse)
...
local function enterOwnedHouse(house)
    ...
    TriggerLocalClientEvent('qb-weed:client:getHousePlants', house)
    entering = false
    setHouseLocations()
...
local function LeaveHouse(house)
    if not FrontCam then
        ...
        exports['qb-interior']:DespawnInterior(houseObj, function()
            UnloadDecorations()
            TriggerLocalClientEvent('qb-weathersync:client:EnableSync')
            SetEntityCoords(GetPlayerPawn(), Config.Houses[CurrentHouse].coords.enter.x, Config.Houses[CurrentHouse].coords.enter.y, Config.Houses[CurrentHouse].coords.enter.z)
            SetEntityHeading(GetPlayerPawn(), Config.Houses[CurrentHouse].coords.enter.h)
            TriggerLocalClientEvent('qb-weed:client:leaveHouse')
            ...
            DeleteBoxTarget('houseExit_' .. house)
...
local function isNearHouses()
    ...
    if ClosestHouse then
        local dist = GetDistanceBetweenCoords(pos, vector3(Config.Houses[ClosestHouse].coords.enter.x, Config.Houses[ClosestHouse].coords.enter.y, Config.Houses[ClosestHouse].coords.enter.z))
        if dist <= 1.5 then
            if HasHouseKey then
                return true
            end
...
exports('qb-houses', 'isNearHouses', isNearHouses)

-- Events

RegisterClientEvent('qb-houses:client:houseMenu', function()
    local housingMenu = {
        {
            isMenuHeader = true,
    ...
    for k, v in pairs(Config.Shells) do
        housingMenu[#housingMenu + 1] = {
            header = v.label .. ' - $' .. v.price,
            txt = 'Purchase and build this house shell.',
            params = {
                isServer = true,
                event = 'qb-houses:server:createHouse',
                args = {
                    shell = k,
                    price = v.price,
                    export = v.export
    ...
    exports['qb-menu']:openMenu(housingMenu)
end)

RegisterClientEvent('qb-houses:client:addGarage', function()
    -- placeholder
end)
...
RegisterClientEvent('qb-houses:server:sethousedecorations', function(house, decorations)
...
RegisterClientEvent('qb-houses:client:sellHouse', function()
    if ClosestHouse and HasHouseKey then
        TriggerServerEvent('qb-houses:server:viewHouse', ClosestHouse)
    end
end)

RegisterClientEvent('qb-houses:client:EnterHouse', function()
    local ped = PlayerPedId()
    local pos = GetEntityCoords(ped)
...
RegisterClientEvent('qb-houses:client:RequestRing', function()
    if ClosestHouse ~= nil then
...
RegisterClientEvent('qb-houses:client:lockHouse', function(bool, house)
    Config.Houses[house].locked = bool
end)

RegisterClientEvent('qb-houses:client:createHouses', function(price, tier)
...
RegisterClientEvent('qb-houses:client:toggleDoorlock', function()
    local pos = GetEntityCoords(PlayerPedId())
    local dist = #(pos - vector3(Config.Houses[ClosestHouse].coords.enter.x, Config.Houses[ClosestHouse].coords.enter.y, Config.Houses[ClosestHouse].coords.enter.z))
    if dist <= 1.5 then
        if HasHouseKey then
...
RegisterClientEvent('qb-houses:client:RingDoor', function(player, house)
...
RegisterClientEvent('qb-houses:client:giveHouseKey', function()
...
RegisterClientEvent('qb-houses:client:removeHouseKey', function()
    if ClosestHouse ~= nil then
        local pedpos = GetEntityCoords(PlayerPedId())
        local housedist = #(pedpos - vector3(Config.Houses[ClosestHouse].coords.enter.x, Config.Houses[ClosestHouse].coords.enter.y, Config.Houses[ClosestHouse].coords.enter.z))
        if housedist <= 5 then
            TriggerCallback('getHouseOwner', function(result)
...
RegisterClientEvent('qb-houses:client:RevokeKey', function(cData)
    RemoveHouseKey(cData.citizenData)
end)

RegisterClientEvent('qb-houses:client:refreshHouse', function()
    Wait(100)
    SetClosestHouse()
end)

RegisterClientEvent('qb-houses:client:SpawnInApartment', function(house)
    local pos = GetEntityCoords(PlayerPedId())
...
RegisterClientEvent('qb-houses:client:enterOwnedHouse', function(house)
...
RegisterClientEvent('qb-houses:client:LastLocationHouse', function(houseId)
    QBCore.Functions.GetPlayerData(function(PlayerData)
        if PlayerData.metadata['injail'] == 0 then
...
RegisterClientEvent('qb-houses:client:setupHouseBlips', function() -- Setup owned on load
    CreateThread(function()
        Wait(2000)
        if isLoggedIn then
            TriggerCallback('getOwnedHouses', function(ownedHouses)
                if ownedHouses then
...
RegisterClientEvent('qb-houses:client:setupHouseBlips2', function() -- Setup unowned on load
...
RegisterClientEvent('qb-houses:client:createBlip', function(coords) -- Create unowned on command
...
RegisterClientEvent('qb-houses:client:refreshBlips', function() -- Refresh unowned on buy
...
RegisterClientEvent('qb-houses:client:SetClosestHouse', function()
    SetClosestHouse()
end)

RegisterClientEvent('qb-houses:client:viewHouse', function(houseprice, brokerfee, bankfee, taxes, firstname, lastname)
...
RegisterClientEvent('qb-houses:client:setLocation', function(cData)
    local ped = PlayerPedId()
    local pos = GetEntityCoords(ped)
...
RegisterClientEvent('qb-houses:client:refreshLocations', function(house, location, type)
...
RegisterClientEvent('qb-houses:client:HomeInvasion', function()
    local ped = PlayerPedId()
    local pos = GetEntityCoords(ped)
    if ClosestHouse ~= nil then
        TriggerCallback('police:server:IsPoliceForcePresent', function(IsPresent)
            if IsPresent then
                local dist = #(pos - vector3(Config.Houses[ClosestHouse].coords.enter.x, Config.Houses[ClosestHouse].coords.enter.y, Config.Houses[ClosestHouse].coords.enter.z))
...
RegisterClientEvent('qb-houses:client:SetRamState', function(bool, house)
    Config.Houses[house].IsRaming = bool
    DeleteHousesTargets()
    SetHousesEntranceTargets()
end)

RegisterClientEvent('qb-houses:client:SetHouseRammed', function(bool, house)
    Config.Houses[house].IsRammed = bool
    DeleteHousesTargets()
    SetHousesEntranceTargets()
end)

RegisterClientEvent('qb-houses:client:ResetHouse', function()
    if ClosestHouse ~= nil then
...
RegisterClientEvent('qb-houses:client:ExitOwnedHouse', function()
...
RegisterClientEvent('qb-houses:client:FrontDoorCam', function()
    local door = vector3(Config.Houses[CurrentHouse].coords.enter.x + POIOffsets.exit.x, Config.Houses[CurrentHouse].coords.enter.y + POIOffsets.exit.y, Config.Houses[CurrentHouse].coords.enter.z - Config.MinZOffset + POIOffsets.exit.z)
    if CheckDistance(door, 1.5) then
...
RegisterClientEvent('qb-houses:client:AnswerDoorbell', function()
...
RegisterClientEvent('qb-houses:client:ViewHouse', function()
...
RegisterClientEvent('qb-houses:client:KeyholderOptions', function(cData)
    optionMenu(cData.citizenData)
end)

RegisterClientEvent('qb-house:client:RefreshHouseTargets', function()
    DeleteHousesTargets()
    SetHousesEntranceTargets()
end)
...
timerHandle = Timer.SetInterval(function()
    if isLoggedIn then
        --TriggerServerEvent('qb-houses:server:setHouses')
        -- TriggerLocalClientEvent('qb-houses:client:setupHouseBlips')
        -- if Config.UnownedBlips then TriggerLocalClientEvent('qb-houses:client:setupHouseBlips2') end
        -- if ClosestHouse and next(Config.Houses[ClosestHouse].garage) then
        --     TriggerLocalClientEvent('qb-garages:client:setHouseGarage', ClosestHouse, HasHouseKey)
        -- end
        -- if not IsInside then SetClosestHouse() end
    end
end, 5000)
