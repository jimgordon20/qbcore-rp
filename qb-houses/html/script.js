...
$("document").ready(function () {
    window.addEventListener("message", function (event) {
        ...
        if (item.type == "buyOption") {
            ...
            $(".decorate-confirm")
                .find("p")
                .html("Are you sure you want to purchase the item for $" + selectedObjectData.price + "?");
        }
    ...
    document.onkeyup = function (data) {
        ...
        if (data.which == 13) {
            if (selectedObjectData != null && $(".decorate-confirm").css("display") != "block") {
                ...
                hEvent("setupMyObjects", JSON.stringify({}), function (myObjects) {
                    $(".decorate-items").html("");
...
Decorations.Close = function () {
    ...
    selectedObjectData = null;
    hEvent("closedecorations", JSON.stringify({}));
};

$(".property-accept").click(function (e) {
    hEvent("buy", JSON.stringify({}));
});

$(".property-cancel").click(function (e) {
    hEvent("exit", JSON.stringify({}));
});
