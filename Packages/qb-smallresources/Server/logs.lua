...
Events.SubscribeRemote('qb-log:server:CreateLog', function(source, name, title, color, message, tagEveryone, imageUrl)
    local postData = {}
    ...
    if #logQueue[name] >= 10 then
        ...
        HTTP.Request(logQueue[name][1].webhook, '/', 'POST', JSON.stringify(postData))
        logQueue[name] = {}
    end
end)

Events.Subscribe('qb-log:server:CreateLog', function(name, title, color, message, tagEveryone, imageUrl)
    local postData = {}
    local tag = tagEveryone or false
    if not Webhooks[name] then
        print('Tried to call a log that isn\'t configured with the name of ' .. name)
...
QBCore.Commands.Add('testwebhook', 'Test Your Discord Webhook For Logs (God Only)', {}, false, function()
    Events.Call('qb-log:server:CreateLog', 'testwebhook', 'Test Webhook', 'default', 'Webhook setup successfully')
end, 'god')
